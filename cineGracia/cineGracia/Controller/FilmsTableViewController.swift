...
class FilmsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        films = manager.selectAll(SQLiteSingleton.getInstance()) as! [Film]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.imageView?.image = UIImage(named: films[indexPath.row].imageName)
        return cell
    }
    ...
    */
    ...
    func setUpExternalDataBAse(){
        ...
        if let docsDir = fileManager.urls(for:.documentDirectory, in: .userDomainMask).first{
            let databaseDeviceURL = docsDir.appendingPathComponent(SQLiteSingleton.dataBaseFileName)
            let databasePath = databaseDeviceURL.absoluteString
            
            if !fileManager.fileExists(atPath: databasePath){
                if let sourceDateBaseURL = Bundle.main.url(forResource: "cineGraciaBBDD", withExtension: "db"){
                    do{
                        try fileManager.copyItem(at: sourceDateBaseURL, to: databaseDeviceURL)
                        print("database copiada")
                    }catch{
                        print("Error copiando la bbdd")
                    }
                }
            }
        }
